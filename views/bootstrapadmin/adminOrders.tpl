	...
	<table class="table table-dark" border="1" cellpadding="1" cellspacing="1">
		...
		{foreach $rsOrders as $item name=orders}
			<tr>
				...
				<td>{$item['id']}</td>
				<td>
					<input class="form-check-input" type="checkbox" id="itemStatus_{$item['id']}" {if $item['status']} checked="checked"{/if} onclick="updateOrderStatus('{$item['id']}');"/>Закрыт
				</td>
				<td>{$item['date_created']}</td>
				<td>
					<input class="form-control" id="datePayment_{$item['id']}" type="text" value="{$item['date_payment']}"/>
					<input class="btn btn-primary" type="button" value="Сохранить" onclick="updateDatePayment('{$item['id']}');"/>
				</td>
				<td>{$item['comment']}</td>
